...
function colorToType(color_string, color_type) {
    ...
    /*
    var rgb_test = /^ *[rR][gG][bB]?\([0-9%, ]+\) *$/;
    if (rgb_test.exec(color_string).length == 1) { return 'rgb'; }

    var hsl_test = /^ *[hH][sS][lL]?\([0-9%, ]+\) *$/;
    if (hsl_test.exec(color_string).length == 1) { return 'hsl'; }

    var hex_test = /^ *#[0-9a-fA-F]{6}|#[0-9a-fA-F]{ *$/;
    if (hex_test.exec(color_string).length === 1) { return 'hex'; }
    */
