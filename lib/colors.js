// Lookup table for the various CSS3 color keywords. Derived from
// "CSS Color Module Level 3", section 4.3 "Extended color keywords" at
// https://www.w3.org/TR/css3-color/#svg-color 
var valid_color_keywords = {
...
/**
 * Validates strings which represent CSS3 colors.
 * @param {string} color_string : string representation of a CSS3 color
 * @return {string} Chrome's interpretation of color_string if valid, else ''
 *     in practice, preserves color keywords, converting all others to RGB
 */
...
/**
 * Converts string which represents CSS3 color to a specified format.
 * @param {string} color_string : string representation of a CSS3 color
 * @param {string} color_type   : format to convert color_string to
 * @return {string} the equivalent of color_string in format of color_type
 */
function colorToType(color_string, color_type) {
    // see https://www.w3.org/TR/css3-color/#rgb-color as ref
    /*
    var hsl_test = /^ *[hH][sS][lL]?\([0-9%, ]+\) *$/;
    if (hsl_test.exec(color_string).length == 1) { return 'hsl'; }
    */
    // Currently only supports conversions to rgb and hex.
    // No support for hsl or alpha masking.
    var valid_color_literals = ["rgb", "hex"];
    if (!valid_color_literals.includes(color_type)) return null;

    var fromRGB = {
        rgb :   function(s) { return s; },
        hex :   function(s) { return rgbToHex(s); }
    }
    var fromStr = {
        rgb :   function(o) { return validateColor(valid_color_keywords[o].rgb); },
        hex :   function(o) { return valid_color_keywords[o].hex; }
    }

    var color = validateColor(color_string);
    if (color === '')           { return null; }
    else if (/rgb/.test(color)) { return fromRGB[color_type](color); }
    else                        { return fromStr[color_type](color); }
}


/**
 * Converts string which represents CSS3 color to RGB format.
 * @param {string} color_string : string representation of a CSS3 color
 * @return {string} the equivalent of color_string in RGB format
 */
function colorToRGB(s) { return colorToType(s, "rgb"); }


/**
 * Converts string which represents CSS3 color to hex format.
 * @param {string} color_string : string representation of a CSS3 color
 * @return {string} the equivalent of color_string in hex format
 */
function colorToHex(s) { return colorToType(s, "hex"); }


/**
 * Converts string with RGB rep. of CSS3 color to hex format.
 * @param {string} color_string : RGB format representation of a CSS3 color
 * @return {string} the equivalent of color_string in hex format
 */
function rgbToHex(rgb_string) {
    function toDecimalInt(x) { return parseInt(x, 10); }
    function toHexString(x)  { return ('0' + x.toString(16)).slice(-2);  }
    return '#' + /[0-9, ]+/.exec(rgb_string)[0]
                           .split(',').map(toDecimalInt)
                           .map(toHexString)
                           .join('');
}
